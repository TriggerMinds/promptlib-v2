...
const PromptCard: React.FC<PromptCardProps> = ({ prompt, onClick, isFavorite = false, onToggleFavorite }) => {
  ...
  return (
    <div 
      ...
      className="group relative bg-white border border-slate-200 rounded-xl p-5 hover:shadow-lg transition-all duration-300 cursor-pointer flex flex-col h-full hover:border-primary/30"
    >
      <div className="flex justify-between items-start mb-3">
        <div className="flex items-center space-x-2">
          <span className={`p-1.5 rounded-md ${prompt.prompt_type === PromptType.IMAGE ? 'bg-pink-100 text-pink-600' : 'bg-slate-100 text-slate-600'}`}>
            {getTypeIcon()}
          </span>
          {prompt.is_featured && (
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {prompt.tags.slice(0, 3).map(tag => (
          <span key={tag} className="text-xs px-2 py-1 bg-slate-50 text-slate-600 rounded-md border border-slate-100 font-medium">
            {tag}
          </span>
        ))}
      </div>
